const game = {
  ...
  // full visibility, unlike console.log() shows only 100 items
  
  //loop around with ForEach 
  // each {} containing the info of the pokemon is an object 

  pokemon.forEach((pokemonx) => {
    if (pokemonx.number === 59) {
        console.log(pokemonx.name);
    }
});
...
game.catchPokemon(pokemon[73]); // for future refs, method(array[indexThatIChoose])
...
game.catchPokemon = function(pokemonObj) {
    ...
    game.items.forEach((pokeLess) => {
        if(pokeLess.name === "pokeball"){    
            pokeLess.quantity = (pokeLess.quantity)-1; // add the existing value because you are already looping overt the item
...
game.gymStatus = function() {
    ...
    game.gyms.forEach((location) => {
        if(location.completed === true ) {
            gymTally.completed++  // ++ no space increase +1 
        } else  { gymTally.incomplete++} // else here is like saying "the rest of the options get this rule"
        }
    )
...
game.partyCounts = function() {
    console.log("Pokemons so far:",game.party.length)
};
game.partyCounts()
...
const topHp = game.party.sort((a, b) => b.hp - a.hp);
